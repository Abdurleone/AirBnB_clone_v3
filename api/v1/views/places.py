...
def filter_places(all_places, condition_func, exclusion_list):
    return list(filter(lambda x: x.id not in exclusion_list, condition_func(all_places)))
...
def find_places():
    ...
    places_id = []
    keys_status = (
        all(['states' in data and type(data['states']) is list, 'states' in data and len(data['states'])]),
        all(['cities' in data and type(data['cities']) is list, 'cities' in data and len(data['cities'])]),
        all(['amenities' in data and type(data['amenities']) is list, 'amenities' in data and len(data['amenities'])])
    )

    places.extend(filter_by_states(all_places, data['states'], places_id))
    places.extend(filter_by_cities(all_places, data['cities'], places_id))
    ...
    if keys_status[2]:
        amenity_ids = [storage.get(Amenity, amenity_id).id for amenity_id in data['amenities'] if amenity_id is not None]
        del_indices = []
